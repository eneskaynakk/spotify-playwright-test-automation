...
test('Spotify Anasayfa İşlemleri', async ({ basePage }) => {
    ...
    await test.step('Spotify anasayfasına gidilir ve doğrulama yapılır',async () => {
        await basePage.goToSpotify();

        await expect(basePage.spotifyLogo).toBeVisible();
    })

    await test.step('Çerezler reddedilir ve kullanıcı oturumu doğrulaması yapılır',async () => {
